...
if sheet_usuarios and sheet_asistencias:
    try:
        ...
        if not df_asistencias.empty and not df_usuarios.empty:
            ...
            # Lógica de Activos vs Inactivos: Códigos de usuarios que tienen al menos 1 asistencia registrada
            ...
            # --- 🛠️ FILA DE 4 INDICADORES CLAVE (AHÍ ARRIBITA) ---
            ...
            with col3:
                with st.container(border=True):
                    # Tu métrica corregida de usuarios que sí han asistido
            ...
            # --- 📋 SECCIÓN NUEVA: TABLA DE CONTROL DE ROLES Y FILTROS ---
            st.subheader("📋 Resumen Estadístico por Rol de Usuario")
            st.write("Consulte el estado de actividad detallado de la comunidad institucional inscrita en el sistema.")

            # Cálculo de la matriz dinámica de Roles (Totales, Activos, Inactivos)
            df_roles_raw = df_usuarios.groupby(['Rol', 'Estado_Actividad']).size().unstack(fill_value=0)
            
            # Asegurar estructuralmente que existan las columnas Sí y No
            ...
            # Componente de Filtro Interactivo para las Directivas
            ...
            # Renderizado estético de la tabla de datos
